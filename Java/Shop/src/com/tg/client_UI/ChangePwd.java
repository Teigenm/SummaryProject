...
public class ChangePwd extends JDialog {
	...
	public ChangePwd(User my_user,JFrame jf) {//界面
		super(jf,true);
	...
	private void uptPwd() {//在数据库中更新密码
		...
		if (udao.changeUser_pwd(my_user.getUsername(),p3)==1) {
	...
	private boolean check() {//验证密码输入是否符合规范
		...
		User eb = new User();
		try {
			eb=udao.queryUserone(my_user.getUsername());
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		new ChangePwd(new User("a","a"),new JFrame()).setVisible(true);
	}
}
