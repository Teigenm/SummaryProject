 ...
 *
 */
public class UpUser2 extends JDialog {
	...
	private String[] roleData = { "", "管理员","业务员" };
	private String[] roleDataState = { "", "在职","离职" };
	private JLabel eid;
	private String[] colName = { "编号", "姓名", "身份证号","性别", "电话", "状态", "权限" };
	//private String[] colName = { "卡号", "姓名", "身份证号","年龄","性别", "地址", "电话", "权限" };
	private JScrollPane sp;

	public UpUser2(JFrame father) {
		super(father, true);
		setTitle("维护业务员信息");
		setSize(750, 400);
		setResizable(false);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {//复位密码
				String idcard = UpUser2.this.eid.getText();
				UserDao udao=new UserDao();
				if (idcard == null || "".equals(idcard)) {
					return;
				}
				if (udao.changeUser_pwd(idcard,"000000")==1) {
					JOptionPane.showMessageDialog(null, "编号"+idcard + "的密码复位成功,复位密码为000000！");
				}
			}
		});
		button.setText("密码复位");
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {//复位业务员，管理员状态
				String idcard = UpUser2.this.eid.getText();
				if (idcard == null || "".equals(idcard)) {
					return;
				}
				if (new UserDao().reSetState(idcard,"1")==1) {
					JOptionPane.showMessageDialog(null,  "编号"+idcard + "的职位状态已复位");
					getListData();
				}
			}
		});
		...
		button_3.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {//修改业务员，管理员信息
				String idcard = UpUser2.this.eid.getText();
				if (idcard == null || "".equals(idcard)) {
				...
				if (check()) {
					update();
					JOptionPane.showMessageDialog(null, "编号"+idcard + "的信息已修改！");
				}
			}
		});
		...
		button_4.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent e) {//删除业务员，管理员
				String eid = UpUser2.this.eid.getText();
 ...
 */
	...
	private boolean check() {//判断业务员，管理员信息是否不符规范
	...
	private void getListData() {//获得业务员，管理员信息并复制于table控件
		List<User> list = new UserDao().getUsers2();
		...
		table.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {//添加table监控并获得idcard
	...
	private void getUser(String idcard) {//根据idcard获得user具体信息
		...
		int index =eb.getUser_type().equals("0")?1:2;
		id.setText(eb.getId());
